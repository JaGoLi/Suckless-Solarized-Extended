...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, iscentered, isfullsize, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow;
	pid_t pid;
	char scratchkey;
	Client *next;
	Client *snext;
...
static void togglefullsize(const Arg *arg);
...
drawbar(Monitor *m)
{
	...
	x = drw_text(drw, x, 0, w, bh, lrpad / 2, separator, 0);
...
tag(const Arg *arg)
{
	if (selmon->sel && arg->ui & TAGMASK) {
		...
		view(arg);
	if (selmon->tagset[1] < (arg->ui & TAGMASK))
		zoom(NULL);
	}
}
...
{
	if (swallowwin) swallowwin = false;

	else swallowwin = true;	

}

void
togglefullsize(const Arg *arg)
{
	Client *c;
	unsigned int found = 0;
...
void viewnext(const Arg *arg) {
	...
	#define TAGSET (selmon->tagset[selmon->seltags])
	
	Arg shift;	

	if (arg->i == +1) shift.ui = (TAGSET << 1);
